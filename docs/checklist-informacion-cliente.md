...
| **B7** | ⛔ **FUERA DE NUESTRO ALCANCE (decisión Oliver, 1-sep):** no lo perseguimos nosotros porque no llevamos las campañas. Queda del lado del cliente. Se anota solo el hecho: Meta se lo va a exigir al publicar anuncios en nicho salud, y lo resolverá quien lleve la publicidad. **No volver a incluirlo en los mensajes al cliente.** *(Contenido original abajo, por si cambia el reparto.)* **Textos legales — requisito ANTES de publicar páginas, no "en el camino":** política de privacidad (que cubra **datos de salud** — la encuesta los captura y hay consumidores UE: RGPD Art. 9), términos, y descargo médico ("esto no sustituye atención médica"). **Los textos los provee el cliente** (su abogado o los que ya usen); nosotros colocamos el descargo estándar y el banner de cookies. *(Si no tienen nada, ofrecer un borrador genérico es decisión de Henry — es trabajo no cotizado con responsabilidad en nicho salud.)* | Sin esto no se publican los funnels: exposición legal real + Meta rechaza anuncios y plantillas en nicho salud. |
